...
final class PauseScheduledReportTool implements JarvisTool
{
    public const NAME = 'pause_scheduled_report';
    ...
    public function definition(): ToolDefinition
    {
        return new ToolDefinition(
            name: self::NAME,
            description: 'Pauses an owned scheduled report.',
            parameters: [
                'type' => 'OBJECT',
                'properties' => [
    ...
    {
        return $this->mutate($call, $context, fn (int $id): ScheduledReport => $this->reports->pauseOwned($context->user, $id));
    }

    private function mutate(ToolCall $call, ToolExecutionContext $context, callable $action): ToolResult
    {
        ...
        try {
            $report = $action((int) $resolved['report']->id);
